...
def prepare_processed_mesh(
    ...
    *,
    ...
    if vertices.shape[0] == 0 or faces.shape[0] == 0:
        return None
    return _sanitize_exact_zero_area_processed_faces(
        np.ascontiguousarray(vertices, dtype=np.float32),
        np.ascontiguousarray(faces, dtype=np.int32),
    )


def _validate_processed_mesh_structure(
    processed_mesh: tuple[np.ndarray, np.ndarray],
) -> tuple[np.ndarray, np.ndarray]:
    if not isinstance(processed_mesh, tuple) or len(processed_mesh) != 2:
        raise TypeError("processed_mesh must be a (vertices, faces) tuple")
    vertices, faces = processed_mesh
    if not isinstance(vertices, np.ndarray) or vertices.dtype != np.float32:
        raise TypeError("processed vertices must be a float32 numpy array")
    if not isinstance(faces, np.ndarray) or faces.dtype != np.int32:
        raise TypeError("processed faces must be an int32 numpy array")
    if vertices.ndim != 2 or vertices.shape[1] != 3 or vertices.shape[0] == 0:
        raise ValueError("processed vertices must have shape [N, 3] with N > 0")
    if faces.ndim != 2 or faces.shape[1] != 3 or faces.shape[0] == 0:
        raise ValueError("processed faces must have shape [F, 3] with F > 0")
    if not np.isfinite(vertices).all():
        raise ValueError("processed vertices must be finite")
    if faces.min() < 0 or faces.max() >= vertices.shape[0]:
        raise ValueError("processed face indices are out of range")
    return np.ascontiguousarray(vertices.copy()), np.ascontiguousarray(faces.copy())


def _exact_zero_area_face_mask(
    vertices: np.ndarray,
    faces: np.ndarray,
) -> np.ndarray:
    ...
    return np.all(doubled_area_vectors == 0.0, axis=1)


def _sanitize_exact_zero_area_processed_faces(
    vertices: np.ndarray,
    faces: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    vertices, faces = _validate_processed_mesh_structure((vertices, faces))
    zero_area_mask = _exact_zero_area_face_mask(vertices, faces)
    kept_faces = faces[~zero_area_mask]
    if kept_faces.shape[0] == 0:
        raise ValueError("processed mesh has no nonzero-area faces")

    referenced_old_indices = np.unique(kept_faces)
    old_to_new = np.full(vertices.shape[0], -1, dtype=np.int32)
    old_to_new[referenced_old_indices] = np.arange(
        referenced_old_indices.shape[0], dtype=np.int32
    )
    return (
        np.ascontiguousarray(vertices[referenced_old_indices], dtype=np.float32),
        np.ascontiguousarray(old_to_new[kept_faces], dtype=np.int32),
    )


def _validate_processed_mesh(
    processed_mesh: tuple[np.ndarray, np.ndarray],
) -> tuple[np.ndarray, np.ndarray]:
    vertices, faces = _validate_processed_mesh_structure(processed_mesh)
    if _exact_zero_area_face_mask(vertices, faces).any():
        raise ValueError("processed faces must have nonzero area")
    return vertices, faces
...
def to_glb(
    ...
    textured: bool = True,
    *,
    processed_mesh: tuple[np.ndarray, np.ndarray] | None = None,
) -> trimesh.Trimesh:
    """
    ...
    if processed_mesh is None:
        processed_mesh = prepare_processed_mesh(
            mesh,
            simplify=simplify,
            fill_holes=fill_holes,
        ...
        if processed_mesh is None:
            return None
    vertices, faces = _validate_processed_mesh(processed_mesh)
